...
- **Only visitors who came from.** A list of sources such as `telegram, facebook, messenger, instagram, tiktok, google`. A visit matches when its link has `?utm_source=` with that name, when it opens inside that app's browser, or when it comes from that site (t.me, facebook.com…). Messenger also counts as facebook; `fb`, `tg`, `ig` are accepted. The sources are remembered for the visit in that browser tab. **Everyone else never sees the popup**, including a plain link opened in a phone browser; the list and the editor warn about this. Telegram on iPhone often gives no sign, so add `?utm_source=telegram` to links posted there.
