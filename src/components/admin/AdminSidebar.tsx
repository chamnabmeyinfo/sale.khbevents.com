...
export default function AdminSidebar() {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#050D09] border-r border-emerald-900/30 text-gray-200">
      {/* 1. Brand & Header */}
      <div className="p-5 border-b border-emerald-950/80 bg-gradient-to-b from-[#08170F] to-[#050D09]">
        <Link href="/admin" className="flex items-center gap-3 group">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-white text-base tracking-wide font-sans">
                KHB <span className="text-amber-400">PORTAL</span>
              </span>
            </div>
            <p className="text-[10px] text-emerald-400/80 font-mono tracking-wider truncate">
              sale.khbevents.com
            </p>
          </div>
        </Link>
        ...
        <div className={`mt-3.5 flex items-center justify-between px-3 py-1.5 rounded-xl border ${
          ...
          <span className="text-[10px] font-semibold text-zinc-400">Full Access</span>
        </div>
      </div>

      {/* 2. Main Aside Feature Menu + Sub Aside Menus */}
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-1.5 scrollbar-thin scrollbar-thumb-emerald-950">
        <div className="px-3 pb-2 text-[10px] font-extrabold uppercase tracking-widest text-emerald-600/70">
          Main Features
        </div>
            ...
            <div key={group.id} className="rounded-xl overflow-hidden mb-1">
              ...
              <div
                className={`flex items-center justify-between px-3 py-2.5 rounded-xl transition-all cursor-pointer ${
                  isGroupActive
                    ? 'bg-gradient-to-r from-emerald-950 to-emerald-900/60 border border-emerald-800/60 text-white font-bold shadow-md'
              ...
              >
                ...
                >
                  <GroupIcon className={`w-4 h-4 shrink-0 ${isGroupActive ? 'text-amber-400' : 'text-zinc-500'}`} />
              ...
              {/* Sub Aside Menu */}
              {isGroupExpanded && (
                <div className="pl-6 pr-1 py-1.5 space-y-1 border-l-2 border-emerald-900/40 ml-4 my-1">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 truncate">
                          <SubIcon className={`w-3.5 h-3.5 shrink-0 ${isSubActive ? 'text-amber-400' : 'text-zinc-500'}`} />
        ...
        <div className="pt-4 mt-4 border-t border-emerald-950/80 px-1 space-y-1">
          <div className="px-2 pb-1.5 text-[10px] font-extrabold uppercase tracking-widest text-emerald-600/70">
            Quick Links
          </div>
          <Link
            href="/"
            target="_blank"
            className="flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium text-emerald-300/80 hover:text-emerald-200 hover:bg-emerald-950/50 transition-all border border-emerald-900/30"
          >
            <div className="flex items-center gap-2">
          ...
          <Link
            ...
            target="_blank"
            className="flex items-center justify-between px-3 py-1.5 rounded-lg text-[11px] text-zinc-400 hover:text-zinc-200 hover:bg-emerald-950/30 transition-all"
          >
        ...
        <div className="flex items-center justify-between gap-3 pt-2 border-t border-emerald-950/60">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-zinc-400 truncate">{userDisplayEmail}</p>
            </div>
          </div>
          ...
          <button
            ...
            title="Sign Out"
            className="p-2 rounded-xl text-zinc-400 hover:text-red-400 hover:bg-red-950/30 transition-all cursor-pointer"
          >
  ...
  return (
    <>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-[#06120B] border-b border-emerald-900/40 px-4 py-3 flex items-center justify-between">
        <button
          onClick={() => setMobileDrawerOpen(true)}
          className="p-2 rounded-xl bg-emerald-950 border border-emerald-800/60 text-white"
        >
      ...
      {mobileDrawerOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex animate-in fade-in duration-200">
          ...
          <div className="relative flex-1 max-w-xs w-full h-full shadow-2xl z-10 animate-in slide-in-from-left duration-200">
            <button
              onClick={() => setMobileDrawerOpen(false)}
              className="absolute right-3 top-3 p-2 rounded-full bg-zinc-900 text-zinc-400 hover:text-white z-20"
            >
              <X className="w-4 h-4" />
            </button>
            {sidebarContent}
